...
private:
    ...
    static bool empty_impl(const Node *node) {
        ...
        // i have NO idea if im doing this right lol
    }
    ...
    static int height_impl(const Node *node) {
        if (!node){
            ...
            return 1 +
            std::max(height_impl(node->left), height_impl(node->right));
        }
    }
    ...
    static Node *copy_nodes_impl(Node *node) {
        if (!node){
            ...
            Node *koala = new Node;
            koala->datum = node->datum;
            koala->left = copy_nodes_impl(node->left);
            koala->right = copy_nodes_impl(node->right);
    ...
    static Node * min_element_impl(Node *node) {
        ...
        // we basically have to go to the left a bunch of times
    ...
    static bool check_sorting_invariant_impl(const Node *node, Compare less) {
        ...
        if (node->left == nullptr && node->right == nullptr) {
        ...
        else if (find_impl(node->right, node->datum, less) != nullptr) {
            return false;
        }
        else if (find_impl(node->left, node->datum, less) != nullptr) {
            return false;
        }
        else if (node->right == nullptr && (less(node->datum,
                                        node->left->datum))) {
            return false;
        }
        else if (node->left == nullptr && (less(node->right->datum,
                                                node->datum))) {
            return false;
        }
            ...
            bool happy = true;
            bool sad = true;
            happy = check_sorting_invariant_impl(node->left, less);
            sad = check_sorting_invariant_impl(node->right, less);
...
//    static bool check_duplicates(const Node *node, Compare less) {
//        Node *copyNode = new Node;
//        copyNode = copy_nodes_impl(node);
//        if ((!&empty_impl) && find_impl(copyNode, copyNode->datum, less) == nullptr) {
//            return false;
//        }
//        else {
//            return true;
...
//    }
    ...
    static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
        ...
        if (!less(val, max_element_impl(node)->datum)) {
            return nullptr;
        }
        if (node->left == nullptr && node->right == nullptr) {
            return node;
        }
//        if ((find_impl(node, val, less))->datum == val) {
//            return node->right;
//        }
        else if (node->left != nullptr && (less(val, node->datum)
                                       && !less(val,node->left->datum))) {
            return node;
        }
        //if value is greater than datum go right
        if (!less(val, node->datum)) {
            return min_greater_than_impl(node->right, val, less);
            }
        //if value is less than datum then go left
        else {
