...
private:
  ...
  public:
      bool operator()(const Pair_type& left, const Pair_type& right) {
          if (key_comp(left.first, right.first)) {
              return true;
          }
          else {
              return false;
          }
//          else if ((left.first == right.first) || (left.first > right.first)){
//              return false;
//          }
//          return false;
      }
...
//    : tree(other.tree) { }
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const {
        std::pair<Key_type, Value_type> kangaroo;
        kangaroo.first = k;
        kangaroo.second = Value_type();
        
        return tree.find(kangaroo);
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
        if (find(k) == tree.end()) {
            ...
            kangaroo.second = Value_type();
            Iterator it1;
            it1 = insert(kangaroo).first;
            return (*it1).second;
            //(*((m.insert(value_type(k, data_type()))).first)).second
            //(*((this->insert(make_pair(k,mapped_type()))).first)).second
//            return insert(kangaroo).first.first.second;
        }
        else {
            Iterator it2 = find(k);
            return (*it2).second;
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val) {
        std::pair<Iterator, bool> monkey;
         if (find(val.first) == tree.end()) {
